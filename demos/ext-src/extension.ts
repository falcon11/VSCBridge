...
class ReactPanel {
	...
	private _getHtmlForWebview() {
		...
		const scriptUri = this._panel.webview.asWebviewUri(scriptPathOnDisk); // scriptPathOnDisk.with({ scheme: 'vscode-resource' });
		const stylePathOnDisk = vscode.Uri.file(path.join(this._extensionPath, 'build', mainStyle));
		const styleUri = this._panel.webview.asWebviewUri(stylePathOnDisk); // stylePathOnDisk.with({ scheme: 'vscode-resource' });
		...
		return `<!DOCTYPE html>
			...
			<head>
				...
				<meta http-equiv="Content-Security-Policy" content="default-src ${this._panel.webview.cspSource} https:; img-src ${this._panel.webview.cspSource} https:; script-src 'nonce-${nonce}';style-src ${this._panel.webview.cspSource} 'unsafe-inline' http: https: data:;">
